...
static String c2dm_msg = "";
    ...
    public void onReceive(Context context, Intent intent) {
     ...
     Toast toast = Toast.makeText(context, "급하게 번개 푸시!\n"+c2dm_msg, Toast.LENGTH_SHORT );
	  toast.setGravity( Gravity.TOP | Gravity.CENTER, 0, 150 );
	  toast.show();
	     
	  NotificationManager notiMgr = (NotificationManager)context.getSystemService(Context.NOTIFICATION_SERVICE);
	
	  Notification noti = new Notification(R.drawable.icon_noti_small, "급하게 번개 푸시", System.currentTimeMillis());

	  
	  PendingIntent content = PendingIntent.getActivity(context, 0,  new Intent(context, BungaeActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP), 0);
	  ...
	  noti.setLatestEventInfo(context, "급하게 번개", c2dm_msg, content);
